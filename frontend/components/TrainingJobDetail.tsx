...
export const TrainingJobDetail: React.FC<TrainingJobDetailProps> = ({ job, onBack }) => {
            ...
            case 'Metrics':
                return (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <MetricChart title="Train vs. Validation Loss" data={job.history} xKey="step" yKey="loss" yUnit="Loss" stroke="#ef4444" />
                        <MetricChart title="Learning Rate vs. Steps" data={job.history} xKey="step" yKey="learning_rate" yUnit="LR" stroke="#3b82f6" />
                        <MetricChart title="Gradient Norm vs. Steps" data={job.history} xKey="step" yKey="grad_norm" yUnit="Norm" stroke="#84cc16" />
                        <MetricChart title="Token Throughput vs. Steps" data={job.history} xKey="step" yKey="throughput" yUnit="tokens/s" stroke="#a855f7" />
                    </div>
                );
            case 'Resources':
                 return (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <MetricChart title="GPU Utilization (%)" data={job.history} xKey="step" yKey="gpu_utilization" yUnit="%" stroke="#f97316" />
                        <MetricChart title="GPU Memory Usage (%)" data={job.history} xKey="step" yKey="gpu_memory_usage" yUnit="%" stroke="#14b8a6" />
                         <MetricChart title="GPU Temperature (°C)" data={job.history} xKey="step" yKey="gpu_temp" yUnit="°C" stroke="#dc2626" />
                        <MetricChart title="Network Bandwidth (MB/s)" data={job.history} xKey="step" yKey="network_bw" yUnit="MB/s" stroke="#6366f1" />
                    </div>
                );
            case 'Artifacts':
        ...
        <div className="space-y-8">
            ...
            <div className="border-b border-gray-200">
                <nav className="-mb-px flex space-x-2" aria-label="Tabs">
                    <TabButton name="Metrics" activeTab={activeTab} setActiveTab={setActiveTab} />
