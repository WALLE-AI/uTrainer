    ...
    <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6 text-sm">
                <div className="md:col-span-2"><p className="text-gray-500">数据集名称</p><p className="font-medium text-gray-800 mt-1">{formData.name || '-'}</p></div>
                <div><p className="text-gray-500">版本</p><p className="font-medium text-gray-800 mt-1">{formData.version || '-'}</p></div>
                <div className="md:col-span-3"><p className="text-gray-500">描述</p><p className="font-medium text-gray-800 mt-1 break-words">{formData.description || '-'}</p></div>
                
                <div><p className="text-gray-500">领域</p><p className="font-medium text-gray-800 mt-1">{formData.domain}</p></div>
                <div><p className="text-gray-500">数据格式</p><p className="font-medium text-gray-800 mt-1">{formData.dataFormat}</p></div>
                <div><p className="text-gray-500">任务类型</p><p className="font-medium text-gray-800 mt-1">{formData.taskType}</p></div>

                <div><p className="text-gray-500">创建者</p><p className="font-medium text-gray-800 mt-1">{formData.creator || '-'}</p></div>
         ...
         <div className="bg-white p-6 rounded-xl border border-gray-200">
            <h2 className="font-semibold text-gray-800 mb-4">数据预览 (前 3 条)</h2>
...
export const UploadDatasetWizard = ({ isOpen, onClose }) => {
    ...
    const renderStepContent = () => {
        switch (step) {
            ...
            case 3: return <Step3_Preview formData={formData} />;
            default: return null;
        }
    };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-6 border-b border-gray-200 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-gray-900">上传与导入数据集</h2>
